(function () {
  ...
  ("use strict");
  angular
    .module("umbraco.install")
    .controller(
      "Umbraco.InstallerController",
  ...
  angular
    ...
    .factory(
      "installerService",
      function ($rootScope, $q, $timeout, $http, $templateRequest) {
        var _status = {
          index: 0,
          current: null,
          steps: null,
  ...
  */
        ...
        var service = {
          ...
          getPackages: function getPackages() {
            return $http.get(
              Umbraco.Sys.ServerVariables.installApiBaseUrl + "GetPackages"
            );
          },
          getSteps: function getSteps() {
            return $http.get(
              Umbraco.Sys.ServerVariables.installApiBaseUrl + "GetSetup"
            );
          },
       ...
       */
          ...
          storeCurrentStep: function storeCurrentStep() {
            _installerModel.instructions[service.status.current.name] =
              service.status.current.model;
          },
          ...
          install: function install() {
            ...
            service.status.feedback = getDescriptionForStepAtIndex(
            ...
            function processInstallStep() {
              $http
                .post(
                  Umbraco.Sys.ServerVariables.installApiBaseUrl +
                    "PostPerformInstall",
                  _installerModel
                )
                .then(
                  function (response) {
                    ...
                    if (!data.complete) {
                      //progress feedback
                      service.status.progress = calculateProgress(
                        service.status.steps,
                        data.nextStep
                      );
                      if (data.view) {
                        ...
                        var desc = getDescriptionForStepName(
                          service.status.steps,
                          data.nextStep
                        );
                        if (desc) {
                  ...
                  function (response) {
                    ...
                    if (status >= 500 && status < 600) {
                      ...
                      $timeout(function () {
                        document
                          .getElementById("ysod")
                          .contentDocument.write(ysod);
                      }, 500);
                    } else {
                      //this is where we handle installer error
                      var v = data.view
                        ? resolveView(data.view)
                        : resolveView("error");
  ...
  angular
    .module("umbraco.install")
    .controller(
      "Umbraco.Installer.DataBaseController",
      function ($scope, $http, installerService) {
        ...
        if (
          Utilities.isUndefined(installerService.status.current.model.dbType) ||
          installerService.status.current.model.dbType === null
        ) {
  ...
  angular
    .module("umbraco.install")
    .controller(
      "Umbraco.Installer.MachineKeyController",
      function ($scope, installerService) {
  ...
  angular
    .module("umbraco.install")
    .controller(
      "Umbraco.Installer.PackagesController",
      function ($scope, installerService) {
  ...
  angular
    .module("umbraco.install")
    .controller(
      "Umbraco.Install.UserController",
      function ($scope, installerService) {
        ...
        if ($scope.installer.current.model.minNonAlphaNumericLength > 0) {
          ...
          for (
            var i = 0;
            i < $scope.installer.current.model.minNonAlphaNumericLength;
            i++
          ) {
            exp += ".*[\\W].*";
          }

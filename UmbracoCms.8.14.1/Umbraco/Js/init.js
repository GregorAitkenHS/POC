...
app.run([
  ...
  function (
    ...
    eventsService.on("app.authenticated", function (evt, data) {
      assetsService._loadInitAssets().then(function () {
        ...
        tourService.registerAllTours().then(function () {
          // Start intro tour
          tourService
            .getTourByAlias("umbIntroIntroduction")
            .then(function (introTour) {
              // start intro tour if it hasn't been completed or disabled
              if (
                introTour &&
                introTour.disabled !== true &&
                introTour.completed !== true
              ) {
                tourService.startTour(introTour);
                localStorageService.set("introTourShown", true);
              } else {
                const introTourShown =
                  localStorageService.get("introTourShown");
                if (!introTourShown) {
                  // Go & show email marketing tour (ONLY when intro tour is completed or been dismissed)
                  tourService
                    .getTourByAlias("umbEmailMarketing")
                    .then(function (emailMarketingTour) {
                      ...
                      if (
                        emailMarketingTour &&
                        emailMarketingTour.disabled !== true &&
                        emailMarketingTour.completed !== true
                      ) {
                        // Only show the email tour once per logged in session
                        // The localstorage key is removed on logout or user session timeout
                        const emailMarketingTourShown = localStorageService.get(
                          "emailMarketingTourShown"
                        );
    ...
    $rootScope.$on("$routeChangeSuccess", function (event, current, previous) {
      var toRetain = currentRouteParams
        ? navigationService.retainQueryStrings(
            currentRouteParams,
            current.params
          )
        : null;
      ...
      if (current.params.section) {
        ...
        var currentSection =
          current.params.section.charAt(0).toUpperCase() +
          current.params.section.slice(1);
        ...
        if (deployEnv) {
          $rootScope.locationTitle =
            deployEnvTitle + "Umbraco - " + $location.$$host;
        }
    ...
    $rootScope.$on(
      ...
      function (event, current, previous, rejection) {
        if (rejection.path) {
          ...
          var returnPath = null;
          if (
            rejection.path == "/login" ||
            rejection.path.startsWith("/login/")
          ) {
    ...
    $rootScope.$on("$routeUpdate", function (event, next) {
      if (!currentRouteParams) {
        ...
        var toRetain = navigationService.retainQueryStrings(
          currentRouteParams,
          next.params
        );
    ...
    var touchDevice =
      /android|webos|iphone|ipad|ipod|blackberry|iemobile|touch/i.test(

      ...
      return {
        ...
        isAuthenticatedAndReady: function (
          ...
          if (
            ...
            ($route.current.params.check === false ||
              $route.current.params.check === "false")
          ) {
            return $q.when(true);
          }
    ...
    $routeProvider
        ...
        controller: function (
          ...
          sectionService.getSectionsForUser().then(function (sections) {
            ...
            var found = _.find(sections, function (s) {
              return (
                s.alias.localeCompare($routeParams.section, undefined, {
                  sensitivity: "accent",
                }) === 0
              );
            });
      ...
      .when("/:section/:tree/:method?", {
        ...
        controller: function ($scope, $routeParams, navigationService) {
          ...
          if (
            $routeParams.section.toLowerCase() === "packages" &&
            $routeParams.tree.toLowerCase() === "packages" &&
            packagePages.indexOf($routeParams.method.toLowerCase()) === -1
          ) {
          ...
          if (
            $routeParams.section.toLowerCase() === "users" &&
            $routeParams.tree.toLowerCase() === "users" &&
            usersPages.indexOf($routeParams.method.toLowerCase()) === -1
          ) {
            $scope.templateUrl = "views/users/overview.html";
            return;
          }
          $scope.templateUrl = navigationService.getTreeTemplateUrl(
            $routeParams.tree,
            $routeParams.method
          );
        },

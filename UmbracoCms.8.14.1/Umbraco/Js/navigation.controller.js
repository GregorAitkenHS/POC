...
function NavigationController(
  ...
  $scope.onTreeInit = function () {
    ...
    $scope.treeApi.callbacks.treeNodeSelect(function (args) {
      ...
      if (
        n.metaData &&
        n.metaData["jsClickCallback"] &&
        Utilities.isString(n.metaData["jsClickCallback"]) &&
        n.metaData["jsClickCallback"] !== ""
      ) {
        ...
        try {
          ...
          $log.error(
            "Error evaluating js callback from legacy tree node: " + ex
          );
        }
      } else if (n.routePath) {
  ...
  evts.push(
    eventsService.on("editors.languages.languageDeleted", function (e, args) {
  ...
  evts.push(
    eventsService.on("editors.languages.languageSaved", function (e, args) {
      if (args.isNew) {
  ...
  evts.push(
    eventsService.on("treeService.removeNode", function (e, args) {
      ...
      if (
        currentEditorState &&
        currentEditorState.id.toString() === args.node.id.toString()
      ) {
    ...
    loadLanguages().then(function (languages) {
      ...
      treeInitPromise.resolve({
        ...
        onLoaded: function () {
          ...
          eventsService.emit("app.navigationReady", {
            treeApi: $scope.treeApi,
          });
        },
      });
    });
  ...
  function lastLanguageOrDefault() {
    ...
    var language = lastCulture
      ? _.find($scope.languages, function (l) {
    ...
    $scope.treeApi
      ...
      .then(function () {
        ...
        if (currNode) {
          ...
          promises.push(
            $scope.treeApi.syncTree({ path: path, activate: true })
          );
        }
...
angular
  .module("umbraco")
  .controller("Umbraco.NavigationController", NavigationController);

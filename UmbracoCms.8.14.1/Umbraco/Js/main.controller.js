...
function MainController(
  ...
  evts.push(
    eventsService.on("app.ready", function (evt, data) {
      ...
      if (
        data.lastUserId !== undefined &&
        data.lastUserId !== null &&
        data.lastUserId !== data.user.id
      ) {
  ...
  // events for search
  evts.push(
    eventsService.on("appState.searchState.changed", function (e, args) {
      if (args.key === "show") {
  ...
  evts.push(
    eventsService.on("appState.drawerState.changed", function (e, args) {
      // set view
      if (args.key === "view") {
  ...
  // events for tours
  evts.push(
    eventsService.on("appState.tour.start", function (name, args) {
      $scope.tour = args;
  ...
  evts.push(
    eventsService.on("appState.tour.end", function () {
      $scope.tour = null;
    })
  );

  evts.push(
    eventsService.on("appState.tour.complete", function () {
      $scope.tour = null;
    })
  );
...
angular
  .module("umbraco")
  .controller("Umbraco.MainController", MainController)

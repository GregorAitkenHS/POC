...
function shortcutCheckKeysPressFirefox(e) {
  if (ctrlDown && keycode == 83) e.preventDefault();
}
...
function runShortCuts() {
  if (currentRichTextObject != undefined && currentRichTextObject != null) {
    if (ctrlDown) {
      ...
      else if (shiftDown && keycode == 9) functionsFrame.tabSwitch(-1);
      ...
      if (shiftDown && currentRichTextObject) {
        if (keycode == 70) {
          functionsFrame.umbracoInsertForm(myAlias);
          return false;
        }
        if (keycode == 76) {
          functionsFrame.umbracoLink(myAlias);
          return false;
        }
        if (keycode == 77) {
          functionsFrame.umbracoInsertMacro(myAlias, umbracoPath);
          return false;
        }
        if (keycode == 80) {
          functionsFrame.umbracoImage(myAlias);
          return false;
        }
        if (keycode == 84) {
          functionsFrame.umbracoInsertTable(myAlias);
          return false;
        }
        if (keycode == 86) {
          functionsFrame.umbracoShowStyles(myAlias);
          return false;
        }
        if (keycode == 85) {
          functionsFrame.document
            .getElementById("TabView1_tab01layer_publish")
            .click();
          return false;
        }
      }
    ...
    if (keycode == 13 && functionsFrame.submitOnEnter != undefined) {
      if (!functionsFrame.disableEnterSubmit) {
        if (functionsFrame.submitOnEnter) {
          // firefox hack
          if (window.addEventListener) e.preventDefault();
          doSubmit();
        }
      }
    }

    ...
    .factory(
      ...
      function (
        ...
        function loadLocales(locales, supportedLocales) {
          ...
          localeUrls = localeUrls.concat(
            getFlatpickrLocales(locales, supportedLocales.flatpickr)
          );
        ...
        function loadLocaleForCurrentUser() {
          userService.getCurrentUser().then(function (currentUser) {
            return javascriptLibraryResource
              .getSupportedLocales()
    ...
    */
          _loadInitAssets: function _loadInitAssets() {
            ...
            if (initAssetsLoaded === false) {
              ...
              return self
                .loadJs(
                  umbRequestHelper.getApiUrl("serverVarsJs", "", ""),
                  $rootScope
                )
                .then(function () {
          ...
          load: function load(pathArray, scope, defaultAssetType) {
            ...
            for (var i = 0; i < assets.length; i++) {
              var asset = assets[i];
              if (asset.path.match(/(\.css$|\.css\?)/gi)) {
                cssAssets.push(asset);
              } else if (asset.path.match(/(\.js$|\.js\?)/gi)) {
                jsAssets.push(asset);
              } else {
 ...
 */
  ...
  (function () {
    ...
    angular.module("umbraco").run([
      ...
      function (clipboardService, udiService) {
        ...
        clipboardService.registerPastePropertyResolver(
          elementTypeBlockResolver,
            ...
            if (
              ...
              if (value.contentData.length > 0) {
                value.contentData.forEach(function (item) {
                  for (var k in item) {
                    propPasteResolverMethod(
                      item[k],
                      clipboardService.TYPES.RAW
                    );
                  }
                });
              }
        ...
        function provideNewUdisForBlockResolver(
          block,
          propPasteResolverMethod
        ) {
          if (block.layout) {
          ...
          if (block.data) {
            ...
            block.data.udi = block.layout.contentUdi =
              udiService.create("element");
          }
          if (block.settingsData) {
            // Make new UDI for settings-element
            block.settingsData.udi = block.layout.settingsUdi =
              udiService.create("element");
          }
        }
        clipboardService.registerPastePropertyResolver(
          provideNewUdisForBlockResolver,
    ...
    function blockEditorService(blockEditorModelObject) {
      ...
      function createModelObject(
        ...
        return new blockEditorModelObject(
          propertyModelValue,
          propertyEditorAlias,
          blockConfigurations,
          scopeOfExistance,
    ...
    angular
      .module("umbraco.services")
      .service("blockEditorService", blockEditorService);
  })();
  ("use strict");
  /**
   * @ngdoc service
  ...
  (function () {
    ...
    function blockEditorModelObjectFactory(
      ...
      function mapToElementModel(elementModel, dataModel) {
        if (
          !elementModel ||
          !elementModel.variants ||
          !elementModel.variants.length
        ) {
          return;
        }
      ...
      function addWatchers(blockObject, isolatedScope, forSettings) {
        ...
        var watcherCreator = forSettings
          ? createSettingsModelPropWatcher
          : createContentModelPropWatcher;
        for (var t = 0; t < variant.tabs.length; t++) {
          var tab = variant.tabs[t];
          for (var p = 0; p < tab.properties.length; p++) {
            var prop = tab.properties[p];
            ...
            blockObject.__watchers.push(
              isolatedScope.$watch(
                "blockObjects._" +
                  ...
                  p +
                  "].value",
                watcherCreator(blockObject, prop)
              )
            );
              ...
              blockObject.__watchers.push(
                isolatedScope.$watch(
                  "blockObjects._" +
                    ...
                    "." +
                    prop.alias,
                  createLayoutSettingsModelWatcher(blockObject, prop)
                )
              );
            } else {
              blockObject.__watchers.push(
                isolatedScope.$watch(
                  "blockObjects._" +
                    ...
                    "." +
                    prop.alias,
                  createDataModelWatcher(blockObject, prop)
                )
              );
            }
          }
      ...
      function formatScaffoldData(scaffold) {
        ...
        if (contentApp) {
          contentApp.view =
            "views/common/infiniteeditors/blockeditor/blockeditor.content.html";
        }
        // remove info app
       ...
       */
      function BlockEditorModelObject(
        propertyModelValue,
        propertyEditorAlias,
        blockConfigurations,
        scopeOfExistance,
        ...
        if (!propertyModelValue) {
          throw new Error(
            "propertyModelValue cannot be undefined, to ensure we keep the binding to the angular model we need minimum an empty object."
          );
        }
        this.__watchers = [];
        this.__labels = {};
        ...
        this.blockConfigurations.forEach(function (blockConfiguration) {
          ...
          if (
            blockConfiguration.stylesheet != null &&
            blockConfiguration.stylesheet !== ""
          ) {
            blockConfiguration.stylesheet =
              umbRequestHelper.convertVirtualToAbsolutePath(
                blockConfiguration.stylesheet
          ...
          if (
            blockConfiguration.thumbnail != null &&
            blockConfiguration.thumbnail !== ""
          ) {
            blockConfiguration.thumbnail =
              umbRequestHelper.convertVirtualToAbsolutePath(
                blockConfiguration.thumbnail
        ...
        this.__watchers.push(
          this.isolatedScope.$on("$destroy", this.destroy.bind(this))
        );
        this.__watchers.push(
          propertyEditorScope.$on(
            "formSubmittingFinalPhase",
            this.sync.bind(this)
          )
        );
      }
      BlockEditorModelObject.prototype = {
        ...
        getBlockConfiguration: function getBlockConfiguration(key) {
          return (
            this.blockConfigurations.find(function (bc) {
        ...
        load: function load() {
          ...
          tasks.push(
            localizationService
              .localize("blockEditor_tabBlockSettings")
          ...
          scaffoldKeys.forEach(function (contentTypeKey) {
            tasks.push(
              contentResource
                .getScaffoldByKey(-20, contentTypeKey)
         ...
         */
        getAvailableAliasesForBlockContent:
          function getAvailableAliasesForBlockContent() {
            var _this = this;
            return this.blockConfigurations.map(function (blockConfiguration) {
              var scaffold = _this.getScaffoldFromKey(
                blockConfiguration.contentElementTypeKey
              );
              if (scaffold) {
         ...
         */
        getAvailableBlocksForBlockPicker:
          function getAvailableBlocksForBlockPicker() {
            var _this2 = this;
            var blocks = [];
            this.blockConfigurations.forEach(function (blockConfiguration) {
              var scaffold = _this2.getScaffoldFromKey(
                blockConfiguration.contentElementTypeKey
              );
              if (scaffold) {
                blocks.push({
        ...
        getBlockObject: function getBlockObject(layoutEntry) {
          ...
          var blockConfiguration = this.getBlockConfiguration(
            dataModel.contentTypeKey
          );
          ...
          if (blockConfiguration === null) {
            console.warn(
              "The block of " +
                contentUdi +
                " is not being initialized because its contentTypeKey('" +
                dataModel.contentTypeKey +
                "') is not allowed for this PropertyEditor"
            );
          } else {
            contentScaffold = this.getScaffoldFromKey(
              blockConfiguration.contentElementTypeKey
            );
            if (contentScaffold === null) {
              console.error(
                "The block of " +
                  contentUdi +
                  " is not begin initialized cause its Element Type was not loaded."
              );
            }
          }
          ...
          if (blockObject.config.label && blockObject.config.label !== "") {
            blockObject.labelInterpolator = $interpolate(
              blockObject.config.label
            );
          }
          ...
          if (blockConfiguration.settingsElementTypeKey) {
            var settingsScaffold = this.getScaffoldFromKey(
              blockConfiguration.settingsElementTypeKey
            );
            if (settingsScaffold !== null) {
              if (!layoutEntry.settingsUdi) {
                ...
                layoutEntry.settingsUdi = createDataEntry(
                  blockConfiguration.settingsElementTypeKey,
                  this.value.settingsData
                );
              }
              var settingsUdi = layoutEntry.settingsUdi;
              var settingsData = getDataByUdi(
                settingsUdi,
                this.value.settingsData
              );
              ...
              appendSettingsContentApp(
                blockObject.content,
                this.__labels.settingsName
              );
            }
          }
         ...
         */
        removeDataAndDestroyModel: function removeDataAndDestroyModel(
          blockObject
        ) {
        ...
        create: function create(contentElementTypeKey) {
          ...
          var entry = {
            contentUdi: createDataEntry(
              contentElementTypeKey,
              this.value.contentData
            ),
          };
          if (blockConfiguration.settingsElementTypeKey != null) {
            entry.settingsUdi = createDataEntry(
              blockConfiguration.settingsElementTypeKey,
              this.value.settingsData
            );
          }
         ...
         */
        createFromElementType: function createFromElementType(
          elementTypeDataModel
        ) {
          elementTypeDataModel = clipboardService.parseContentForPaste(
            elementTypeDataModel,
            clipboardService.TYPES.ELEMENT_TYPE
          );
          ...
          var dataModel = getDataByUdi(
            layoutEntry.contentUdi,
            this.value.contentData
          );
          if (dataModel === null) {
            return null;
        ...
        createFromBlockData: function createFromBlockData(blockData) {
          blockData = clipboardService.parseContentForPaste(
            blockData,
            clipboardService.TYPES.BLOCK
          );
          ...
          if (blockData.settingsData) {
            ...
            if (blockConfiguration.settingsElementTypeKey) {
              ...
              if (
                blockConfiguration.settingsElementTypeKey ===
                blockData.settingsData.contentTypeKey
              ) {
                this.value.settingsData.push(blockData.settingsData);
              } else {
                notificationsService.error(
                  "Clipboard",
                  "Couldn't paste because settings-data is not compatible."
                );
                return null;
              }
            } else {
    ...
    angular
      .module("umbraco.services")
      .service("blockEditorModelObject", blockEditorModelObjectFactory);
  })();
  ("use strict");
  /**
   * @ngdoc service
  ...
  function clipboardService(
    ...
    clipboardTypeResolvers[TYPES.ELEMENT_TYPE] = function (
      element,
      propMethod
    ) {
    ...
    */
    ...
    var prepareEntryForStorage = function prepareEntryForStorage(
      type,
      entryData,
      firstLevelClearupMethod
    ) {
      ...
      if (typeResolver) {
        ...
        console.warn(
          "Umbraco.service.clipboardService has no type resolver for '" +
            type +
            "'."
        );
      }
    ...
    var isEntryCompatible = function isEntryCompatible(
      entry,
      type,
      allowedAliases
    ) {
      return (
        ...
        (allowedAliases === null ||
          (entry.alias &&
            allowedAliases.filter(function (allowedAlias) {
              return allowedAlias === entry.alias;
            }).length > 0) ||
          (entry.aliases &&
            entry.aliases.filter(function (entryAlias) {
              return (
                allowedAliases.filter(function (allowedAlias) {
    ...
    service.copy = function (
      ...
      uniqueKey =
        uniqueKey ||
        data.key ||
        console.error("missing unique key for this content");
      ...
      if (saveStorage(storage) === true) {
        ...
        notificationsService.error(
          "Clipboard",
          "Couldnt copy this data to clipboard."
        );
      }
    };
    /**
  ...
  angular
    .module("umbraco.services")
    .factory("contentAppHelper", contentAppHelper);
  ("use strict");
  ...
  function contentEditingHelper(
    ...
    function showNotificationsForModelsState(ms, messageType) {
      ...
      for (
        var _i = 0, _Object$entries = Object.entries(ms);
        ...
        if (
          (Utilities.isString(errorMsg) && errorMsg.startsWith("[")) ||
          Utilities.isArray(errorMsg)
        ) {
          // flatten the json structure, create validation paths for each property and add each as a property error
          var idsToErrors = serverValidationManager.parseComplexEditorError(
            errorMsg,
            ""
          );
    ...
    return {
      ...
      contentEditorPerformSave: function contentEditorPerformSave(args) {
        ...
        var _rebindCallback =
          args.rebindCallback === undefined
            ? self.reBindChangedProperties
            : args.rebindCallback;
                ...
                if (!args.infiniteMode) {
                  self.handleSuccessfulSave({
                    ...
                    rebindCallback: function rebindCallback() {
                      _rebindCallback.apply(self, [args.content, data]);
                    },
                  });
              ...
              function (err) {
                ...
                self.handleSaveError({
                  ...
                  rebindCallback: function rebindCallback() {
                    ...
                    if (err.data) {
                      _rebindCallback.apply(self, [args.content, err.data]);
                    }
                  },
                });
                //update editor state to what is current
    ...
    here we'll build the buttons according to the chars of the user. */
      configureContentEditorButtons: function configureContentEditorButtons(
        args
      ) {
        if (!Utilities.isObject(args)) {
          throw "args must be an object";
        ...
        if (
          !args.methods.saveAndPublish ||
          !args.methods.sendToPublish ||
          !args.methods.unpublish ||
          !args.methods.schedulePublish ||
          !args.methods.publishDescendants
        ) {
          ...
          switch (ch) {
            case "U":
              ...
              return {
                ...
                addEllipsis:
                  args.content.variants && args.content.variants.length > 1
                    ? "true"
                    : "false",
              };
            case "H":
        ...
        if (!args.create || _.contains(args.content.allowedActions, "C")) {
          ...
          if (
            ...
            args.create &&
            _.contains(args.content.allowedActions, "C")
          ) {
        ...
        if (buttons.defaultButton) {
          ...
          if (_.contains(args.content.allowedActions, "U")) {
            ...
            buttons.subButtons.push(
              createButtonDefinition("PUBLISH_DESCENDANTS")
            );
          }
          ...
          if (!args.create) {
            var hasPublishedVariant =
              args.content.variants.filter(function (variant) {
                return (
                  variant.state === "Published" ||
                  variant.state === "PublishedPendingChanges"
                );
              }).length > 0;
            if (
              hasPublishedVariant &&
              _.contains(args.content.allowedActions, "U") &&
              _.contains(args.content.allowedActions, "Z")
            ) {
      ...
      buildCompositeVariantId: function buildCompositeVariantId(variant) {
        return (
          (variant.language ? variant.language.culture : "invariant") +
          "_" +
          (variant.segment ? variant.segment : "null")
        );
      },
      /**
       * @ngdoc method
      ...
      getAllowedActions: function getAllowedActions(content, creating) {
        ...
        if (defaultAction) {
          ...
          if (!creating) {
            if (
              content.publishDate &&
              _.contains(content.allowedActions, "U")
            ) {
              actions.push("Z");
            }
          }
       ...
       */
      reBindChangedProperties: function reBindChangedProperties(
        origContent,
        savedContent
      ) {
        ...
        if (args.err.status === 400) {
          ...
          if (args.err.data && args.err.data.ModelState) {
            ...
            if (args.showNotifications) {
              showNotificationsForModelsState(
                args.err.data.ModelState,
                messageType
              );
            }
            if (
              !this.redirectToCreatedContent(
                args.err.data.id,
                args.softRedirect
              ) ||
              args.softRedirect
              ...
              if (
                args.rebindCallback &&
                Utilities.isFunction(args.rebindCallback)
              ) {
                args.rebindCallback();
              }
      ...
      handleSuccessfulSave: function handleSuccessfulSave(args) {
        ...
        if (
          !this.redirectToCreatedContent(
            args.redirectId ? args.redirectId : args.savedContent.id,
            args.softRedirect
          ) ||
          args.softRedirect
        ) {
       ...
       */
      redirectToCreatedContent: function redirectToCreatedContent(
        id,
        softRedirect
      ) {
      ...
      sortVariants: function sortVariants(a, b) {
        ...
        var compareDefault = function compareDefault(a, b) {
          return (
            (a.language && a.language.isDefault ? -1 : 1) -
            (b.language && b.language.isDefault ? -1 : 1)
          );
        };
        ...
        var compareMandatory = function compareMandatory(a, b) {
          return a.state === "PublishedPendingChanges" ||
            a.state === "Published"
            ? 0
            : (a.language && a.language.isMandatory ? -1 : 1) -
                (b.language && b.language.isMandatory ? -1 : 1);
        };
       ...
       *
       */
      getSortedVariantsAndSegments: function getSortedVariantsAndSegments(
        variantsAndSegments
      ) {
        var _this = this;
        var sortedVariants = variantsAndSegments
          .filter(function (variant) {
            return !variant.segment;
          })
          .sort(this.sortVariants);
        var variantsWithSegments = variantsAndSegments.filter(function (
          variant
        ) {
          return variant.segment;
        });
        ...
        sortedVariants.forEach(function (variant) {
          var sortedMatchedSegments = variantsWithSegments
            .filter(function (segment) {
              return (
                segment.language &&
                variant.language &&
                segment.language.culture === variant.language.culture
              );
            })
            .sort(_this.sortVariants);
          // remove variants for this culture
          variantsWithSegments = variantsWithSegments.filter(function (
            segment
          ) {
            return (
              !segment.language ||
          ...
          sortedAvailableVariants = [].concat(
            _toConsumableArray(sortedAvailableVariants),
            [variant],
            _toConsumableArray(sortedMatchedSegments)
          );
        });
        // if we have segments without a parent language variant we need to add the remaining variantsWithSegments to the array
        sortedAvailableVariants = [].concat(
          _toConsumableArray(sortedAvailableVariants),
          _toConsumableArray(variantsWithSegments.sort(this.sortVariants))
        );
  ...
  angular
    .module("umbraco.services")
    .factory("contentEditingHelper", contentEditingHelper);
  ("use strict");
  /**
   * @ngdoc service
  ...
  function contentTypeHelper(
    ...
    var contentTypeHelperService = {
      ...
      generateModels: function generateModels() {
        var deferred = $q.defer();
        var modelsResource = $injector.has("modelsBuilderManagementResource")
          ? $injector.get("modelsBuilderManagementResource")
          : null;
        var modelsBuilderEnabled =
          Umbraco.Sys.ServerVariables.umbracoPlugins.modelsBuilder.enabled;
        if (modelsBuilderEnabled && modelsResource) {
          modelsResource.buildModels().then(
            function (result) {
      ...
      checkModelsBuilderStatus: function checkModelsBuilderStatus() {
        ...
        var modelsBuilderEnabled =
          ...
          Umbraco.Sys.ServerVariables.umbracoPlugins &&
          Umbraco.Sys.ServerVariables.umbracoPlugins.modelsBuilder &&
          Umbraco.Sys.ServerVariables.umbracoPlugins.modelsBuilder.enabled ===
            true;
        if (modelsBuilderEnabled && modelsResource) {
      ...
      makeObjectArrayFromId: function makeObjectArrayFromId(
        idArray,
        objectArray
      ) {
        var newArray = [];
        for (var idIndex = 0; idArray.length > idIndex; idIndex++) {
          ...
          for (
            var objectIndex = 0;
            objectArray.length > objectIndex;
            objectIndex++
          ) {
      ...
      validateAddingComposition: function validateAddingComposition(
        contentType,
        compositeContentType
      ) {
        //Validate that by adding this group that we are not adding duplicate property type aliases
        var propertiesAdding = _.flatten(
          _.map(compositeContentType.groups, function (g) {
        ...
        var propAliasesExisting = _.filter(
          _.flatten(
            _.map(contentType.groups, function (g) {
              return _.map(g.properties, function (p) {
                return p.alias;
              });
            })
      ...
      mergeCompositeContentType: function mergeCompositeContentType(
        contentType,
        compositeContentType
      ) {
        //Validate that there are no overlapping aliases
        var overlappingAliases = this.validateAddingComposition(
          contentType,
          compositeContentType
        );
        if (overlappingAliases.length > 0) {
          throw new Error(
            "Cannot add this composition, these properties already exist on the content type: " +
              overlappingAliases.join()
          );
        }
        compositeContentType.groups.forEach(function (compositionGroup) {
          // order composition groups based on sort order
          compositionGroup.properties = $filter("orderBy")(
            compositionGroup.properties,
            "sortOrder"
          );
          // get data type details
          compositionGroup.properties.forEach(function (property) {
            dataTypeResource
              .getById(property.dataTypeId)
          ...
          contentType.groups.forEach(function (contentTypeGroup) {
            if (contentTypeGroup.name === compositionGroup.name) {
              ...
              contentTypeGroup.properties = compositionGroup.properties.concat(
                contentTypeGroup.properties
              );
              // update sort order on all properties in merged group
              contentTypeGroup.properties =
                contentTypeHelperService.updatePropertiesSortOrder(
                  contentTypeGroup.properties
                );
              // make parentTabContentTypeNames to an array so we can push values
              if (
                contentTypeGroup.parentTabContentTypeNames === null ||
                contentTypeGroup.parentTabContentTypeNames === undefined
              ) {
                contentTypeGroup.parentTabContentTypeNames = [];
              }
              // push name to array of merged composite content types
              contentTypeGroup.parentTabContentTypeNames.push(
                compositeContentType.name
              );
              // make parentTabContentTypes to an array so we can push values
              if (
                contentTypeGroup.parentTabContentTypes === null ||
                contentTypeGroup.parentTabContentTypes === undefined
              ) {
                contentTypeGroup.parentTabContentTypes = [];
              }
              // push id to array of merged composite content types
              contentTypeGroup.parentTabContentTypes.push(
                compositeContentType.id
              );
          ...
          if (
            compositionGroup.groupIsMerged === false ||
            compositionGroup.groupIsMerged === undefined
          ) {
            // make parentTabContentTypeNames to an array so we can push values
            if (
              compositionGroup.parentTabContentTypeNames === null ||
              compositionGroup.parentTabContentTypeNames === undefined
            ) {
              compositionGroup.parentTabContentTypeNames = [];
            }
            // push name to array of merged composite content types
            compositionGroup.parentTabContentTypeNames.push(
              compositeContentType.name
            );
            // make parentTabContentTypes to an array so we can push values
            if (
              compositionGroup.parentTabContentTypes === null ||
              compositionGroup.parentTabContentTypes === undefined
            ) {
              compositionGroup.parentTabContentTypes = [];
            }
            // push id to array of merged composite content types
            compositionGroup.parentTabContentTypes.push(
              compositeContentType.id
            );
        ...
        contentType.groups = $filter("orderBy")(
          contentType.groups,
          "sortOrder"
        );
        return contentType;
      },
      splitCompositeContentType: function splitCompositeContentType(
        contentType,
        compositeContentType
      ) {
        var groups = [];
        contentType.groups.forEach(function (contentTypeGroup) {
          if (contentTypeGroup.tabState !== "init") {
            var idIndex = contentTypeGroup.parentTabContentTypes.indexOf(
              compositeContentType.id
            );
            var nameIndex = contentTypeGroup.parentTabContentTypeNames.indexOf(
              compositeContentType.name
            );
      ...
      updatePropertiesSortOrder: function updatePropertiesSortOrder(
        properties
      ) {
        var sortOrder = 0;
      ...
      insertDefaultTemplatePlaceholder:
        function insertDefaultTemplatePlaceholder(defaultTemplate) {
          // get template placeholder
          var templatePlaceholder =
            contentTypeHelperService.getTemplatePlaceholder();
      ...
      insertChildNodePlaceholder: function insertChildNodePlaceholder(
        array,
        name,
        icon,
        id
  ...
  angular
    .module("umbraco.services")
    .factory("contentTypeHelper", contentTypeHelper);
  ("use strict");
  /**
   * @ngdoc service
  ...
  function cropperHelper(umbRequestHelper, $http) {
    var service = {
      ...
      calculateAspectRatioFit: function calculateAspectRatioFit(
        srcWidth,
        srcHeight,
        maxWidth,
        maxHeight,
      ...
      calculateSizeToRatio: function calculateSizeToRatio(
        srcWidth,
        srcHeight,
        ratio
      ) {
      ...
      scaleToMaxSize: function scaleToMaxSize(
        srcWidth,
        srcHeight,
        maxWidth,
        maxHeight
      ...
      convertToStyle: function convertToStyle(
        coordinates,
        originalSize,
        viewPort,
        offset
      ) {
        var coordinates_px = service.coordinatesToPixels(
          coordinates,
          originalSize,
          offset
        );
      ...
      coordinatesToPixels: function coordinatesToPixels(
        coordinates,
        originalSize,
        offset
      ) {
      ...
      pixelsToCoordinates: function pixelsToCoordinates(
        image,
        width,
        height,
        offset
 ...
 */
    ...
    function editorService(eventsService, keyboardService, $timeout) {
      ...
      */
      ...
      function mediaPicker(editor) {
        editor.view =
          "views/common/infiniteeditors/mediapicker/mediapicker.html";
        if (!editor.size) editor.size = "medium";
      ...
      function mediaCropDetails(editor) {
        editor.view =
          "views/common/infiniteeditors/mediapicker/overlays/mediacropdetails.html";
        open(editor);
      }
      /**
       * @ngdoc method
      ...
      function queryBuilder(editor) {
        editor.view =
          "views/common/infiniteeditors/querybuilder/querybuilder.html";
        open(editor);
      }
      /**
       * @ngdoc method
      ...
      function nodePermissions(editor) {
        editor.view =
          "views/common/infiniteeditors/nodepermissions/nodepermissions.html";
        if (!editor.size) editor.size = "small";
        open(editor);
      }
      /**
      ...
      function insertCodeSnippet(editor) {
        editor.view =
          "views/common/infiniteeditors/insertcodesnippet/insertcodesnippet.html";
        if (!editor.size) editor.size = "small";
        open(editor);
      }
      /**
      ...
      function userGroupPicker(editor) {
        editor.view =
          "views/common/infiniteeditors/usergrouppicker/usergrouppicker.html";
        if (!editor.size) editor.size = "small";
        open(editor);
      }
      /**
      ...
      function sectionPicker(editor) {
        editor.view =
          "views/common/infiniteeditors/sectionpicker/sectionpicker.html";
        if (!editor.size) editor.size = "small";
        open(editor);
      }
      /**
      ...
      function insertField(editor) {
        editor.view =
          "views/common/infiniteeditors/insertfield/insertfield.html";
        if (!editor.size) editor.size = "small";
        open(editor);
      }
      /**
      ...
      function memberGroupPicker(editor) {
        editor.view =
          "views/common/infiniteeditors/membergrouppicker/membergrouppicker.html";
        if (!editor.size) editor.size = "small";
        open(editor);
      }
      /**
      ...
      function unbindKeyboardShortcuts() {
        ...
        for (
          var _i = 0, _Object$entries = Object.entries(shortcuts);
          _i < _Object$entries.length;
          _i++
        ) {
          var _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2),
      ...
      function rebindKeyboardShortcuts() {
        ...
        var lastSetOfShortcuts =
          editorsKeyboardShorcuts[lastSetOfShortcutsIndex];
        // rebind shortcuts
        for (
          var _i2 = 0, _Object$entries2 = Object.entries(lastSetOfShortcuts);
  ...
  angular
    .module("umbraco.services")
    .factory("editorState", function ($rootScope, eventsService) {
      var current = null;
      var state = {
        /**
         * @ngdoc function
...
*/
  ...
  function externalLoginInfoService(externalLoginInfo, umbRequestHelper) {
    ...
    function getLoginProviderView(provider) {
      if (
        provider &&
        provider.properties &&
        provider.properties.UmbracoBackOfficeExternalLoginOptions &&
        provider.properties.UmbracoBackOfficeExternalLoginOptions
          .CustomBackOfficeView
      ) {
        return umbRequestHelper.convertVirtualToAbsolutePath(
          provider.properties.UmbracoBackOfficeExternalLoginOptions
            .CustomBackOfficeView
        );
      }
    ...
    function hasDenyLocalLogin(provider) {
      if (!provider) {
        return _.some(externalLoginInfo.providers, function (x) {
          return (
            x.properties &&
            x.properties.UmbracoBackOfficeExternalLoginOptions &&
            x.properties.UmbracoBackOfficeExternalLoginOptions
              .DenyLocalLogin === true
          );
        });
  ...
  angular
    .module("umbraco.services")
    .factory("externalLoginInfoService", externalLoginInfoService);
  ("use strict");
  /**
   * @ngdoc service
    ...
    var mgr = {
      ...
      setFiles: function setFiles(args) {
        ...
        fileCollection = _.reject(fileCollection, function (item) {
          return (
            item.alias === args.propertyAlias &&
            (!args.culture || args.culture === item.culture) &&
            (!args.segment || args.segment === item.segment)
          );
        });
 ...
 */
  ...
  (function () {
    ...
    angular
      .module("umbraco.services")
      .factory("focusLockService", focusLockService);
  })();
  ("use strict");
  function _toConsumableArray(arr) {
    return (
  ...
  function formHelper(
    ...
    return {
      ...
      focusOnFirstError: function focusOnFirstError(form) {
        var invalidNgForms = form.$$element.find(
          ".umb-property ng-form.ng-invalid, .umb-property-editor ng-form.ng-invalid-required"
        );
        var firstInvalidNgForm = invalidNgForms.first();
        if (firstInvalidNgForm.length !== 0) {
          var focusableFields = _toConsumableArray(
            firstInvalidNgForm.find(
              "umb-range-slider .noUi-handle,input,textarea,select,button"
            )
          );
          if (focusableFields.length !== 0) {
            var firstErrorEl = focusableFields.find(function (el) {
              return (
                el.type !== "hidden" && el.hasAttribute("readonly") === false
              );
            });
      ...
      resetForm: function resetForm(args) {
        ...
        args.scope.$broadcast(
          args.hasErrors ? "formSubmittedValidationFailed" : "formSubmitted",
          { scope: args.scope }
        );
      },
  ...
  angular
    ...
    .factory(
      "helpService",
      function ($http, $q, umbRequestHelper, dashboardResource) {
        var helpTopics = {};
        ...
        function fetchUrl(site, url) {
          ...
          if (found) {
            ...
            dashboardResource.getRemoteXmlData(site, url).then(
              function (data) {
                var feed = $(data.data);
                var topics = [];
        ...
        var service = {
          ...
          getContextHelpForPage: function getContextHelpForPage(
            section,
            tree,
            baseurl
          ) {
  ...
  angular
    ...
    .factory(
      "historyService",
      function ($rootScope, $timeout, angularHelper, eventsService) {
        var nArray = [];
        function _add(item) {
          if (!item) {
            return null;
        ...
        return {
           ...
           */
          getLastAccessedItemForSection: function getLastAccessedItemForSection(
            sectionAlias
          ) {
  ...
  */
    ...
    function init() {
      ...
      $http({
        method: "GET",
        url:
          Umbraco.Sys.ServerVariables.umbracoUrls.iconApiBaseUrl + "GetIcons",
      }).then(
        function (response) {
          resourceLoadStatus = "loaded";
          for (
            var _i = 0, _Object$entries = Object.entries(response.data.Data);
            _i < _Object$entries.length;
            _i++
          ) {
            var _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2),
    ...
    return {
      ...
      formatContentTypeThumbnails: function formatContentTypeThumbnails(
        contentTypes
      ) {
        for (var i = 0; i < contentTypes.length; i++) {
          if (
            contentTypes[i].thumbnailIsClass === undefined ||
            contentTypes[i].thumbnailIsClass
          ) {
            contentTypes[i].cssClass = this.convertFromLegacyIcon(
              contentTypes[i].thumbnail
            );
          } else {
            contentTypes[i].style =
              "background-image: url('" +
              contentTypes[i].thumbnailFilePath +
              "');height:36px; background-position:4px 0px; background-repeat: no-repeat;background-size: 35px 35px;";
      ...
      formatContentTypeIcons: function formatContentTypeIcons(contentTypes) {
        for (var i = 0; i < contentTypes.length; i++) {
          if (!contentTypes[i].icon) {
            ...
            contentTypes[i].icon = this.convertFromLegacyIcon(
              contentTypes[i].icon
            );
          }
      ...
      isFileBasedIcon: function isFileBasedIcon(icon) {
        ...
        if (
          icon.startsWith("..") ||
          (!icon.startsWith(".") && icon.indexOf(".") > 1)
        ) {
          return true;
        }
        return false;
      ...
      convertFromLegacyTreeNodeIcon: function convertFromLegacyTreeNodeIcon(
        treeNode
      ) {
        ...
        if (
          ...
          if (opt["inputDisabled"]) {
            ...
            if (
              ...
              elt.hasAttribute("disable-hotkeys")
            ) {
  ...
  angular
    .module("umbraco.services")
    .factory("keyboardService", ["$window", "$timeout", keyboardService]);
  ("use strict");
  /**
 @ngdoc service
 ...
 */
  (function () {
    ...
    function listViewHelper(
      ...
      function selectItem(item, selection) {
        ...
        for (var i = 0; selection.length > i; i++) {
          ...
          if (
            (item.id !== 2147483647 && item.id === selectedItem.id) ||
            (item.key && item.key === selectedItem.key)
          ) {
            isSelected = true;
          }
        }
      ...
      function deselectItem(item, selection) {
        for (var i = 0; selection.length > i; i++) {
          ...
          if (
            ...
            item.selected = false;
            $rootScope.$broadcast("listView.itemsChanged", {
              items: selection,
            });
          }
        }
      ...
      function isSelectedAll(items, selection) {
        ...
        for (var itemIndex = 0; items.length > itemIndex; itemIndex++) {
          ...
          for (
            var selectedIndex = 0;
            selection.length > selectedIndex;
            selectedIndex++
          ) {
      ...
      function setSortingDirection(col, direction, options) {
        return (
          options.orderBy.toUpperCase() === col.toUpperCase() &&
    ...
    angular
      .module("umbraco.services")
      .factory("listViewHelper", listViewHelper);
  })();
  ("use strict");
  /**
 @ngdoc service
 ...
 */
  (function () {
    ...
    angular
      .module("umbraco.services")
      .factory("listViewPrevalueHelper", listViewPrevalueHelper);
  })();
  ("use strict");
  /**
   * @ngdoc service
  ...
  angular
    ...
    .factory(
      "localizationService",
      function ($http, $q, eventsService, $window, $filter, userService) {
  ...
  function macroService() {
    return {
      ...
      parseMacroSyntax: function parseMacroSyntax(syntax) {
        ...
        var paramsChunk = match[1]
          .trim()
          .replace(
            new RegExp("UMBRACO_MACRO macroAlias=[\"']" + alias + "[\"']"),
            ""
          )
          .trim();
      ...
      generateMacroSyntax: function generateMacroSyntax(args) {
        ...
        var macroString =
          '<?UMBRACO_MACRO macroAlias="' + args.macroAlias + '" ';
      ...
      collectValueData: function collectValueData(
        macro,
        macroParams,
        renderingEngine
      ) {
    ...
    return {
      ...
      getMediaPropertyValue: function getMediaPropertyValue(options) {
        ...
        var imageProp = _.find(props, function (item) {
          ...
          if (
            Utilities.isString(item.value) &&
            item.value.indexOf(mediaRoot) === 0
          ) {
            return true;
          }
          return false;
        ...
        if (mediaVal.length && mediaVal.length > 0) {
          if (
            !options.imageOnly ||
            (options.imageOnly === true && mediaVal[0].isImage)
          ) {
      ...
      getImagePropertyValue: function getImagePropertyValue(options) {
        ...
        options.mediaModel = options.imageModel
          ? options.imageModel
          : options.mediaModel;
      ...
      registerFileResolver: function registerFileResolver(
        propertyEditorAlias,
        func
      ) {
       ...
       */
      resolveFileFromEntity: function resolveFileFromEntity(
        mediaEntity,
        thumbnail
      ) {
        var mediaPath = Utilities.isObject(mediaEntity.metaData)
          ? mediaEntity.metaData.MediaPath
          : null;
        if (!mediaPath) {
          //don't throw since this image legitimately might not contain a media path, but output a warning
          $log.warn(
            "Cannot resolve the file url from the mediaEntity, it does not contain the required metaData"
          );
          return null;
        }
        if (thumbnail) {
      ...
      resolveFile: function resolveFile(mediaItem, thumbnail) {
        function iterateProps(props) {
          ...
          for (var resolver in _mediaFileResolvers) {
            ...
            if (property) {
              res = _mediaFileResolvers[resolver](
                property,
                mediaItem,
                thumbnail
              );
      ...
      getThumbnailFromPath: function getThumbnailFromPath(imagePath) {
        ...
        var thumbnailUrl =
          umbRequestHelper.getApiUrl("imagesApiBaseUrl", "GetBigThumbnail", [
  ...
  function mediaTypeHelper(mediaTypeResource, $q) {
    var mediaTypeHelperService = {
      ...
      getTypeAcceptingFileExtensions: function getTypeAcceptingFileExtensions(
        mediaTypes,
        fileExtensions
      ) {
        return mediaTypes.filter(function (mediaType) {
          ...
          if (uploadProperty) {
            ...
            if (uploadProperty.editor === "Umbraco.ImageCropper") {
              ...
              acceptedFileExtensions =
                uploadProperty.config.fileExtensions &&
                uploadProperty.config.fileExtensions.length > 0
                  ? uploadProperty.config.fileExtensions.map(function (x) {
                      return x.value;
                    })
                  : null;
            }
            if (acceptedFileExtensions && acceptedFileExtensions.length > 0) {
              return (
                ...
                fileExtensions.filter(function (fileExt) {
  ...
  angular
    .module("umbraco.services")
    .factory("mediaTypeHelper", mediaTypeHelper);
  ("use strict");
  /**
   * @ngdoc service
  ...
  function umbracoMenuActions(
    ...
    return {
      ExportMember: function ExportMember(args) {
        ...
        umbRequestHelper.downloadFile(url).then(
          function () {
            localizationService
      ...
      DisableUser: function DisableUser(args) {
        localizationService
          .localize("defaultdialogs_confirmdisable")
          .then(function (txtConfirmDisable) {
            var currentMenuNode = UmbClientMgr.mainTree().getActionNode();
            if (
              confirm(txtConfirmDisable + ' "' + args.entity.name + '"?\n\n')
            ) {
              usersResource.disableUser(args.entity.id).then(function () {
                navigationService.syncTree({
                  tree: args.treeAlias,
                  path: [args.entity.parentId, args.entity.id],
                  forceReload: true,
                });
              });
            }
      ...
      RefreshNode: function RefreshNode(args) {
        ...
        if (treeRoot && treeRoot.root) {
          var treeNode = treeService.getDescendantNode(
            treeRoot.root,
            args.entity.id,
            args.treeAlias
          );
      ...
      CreateChildEntity: function CreateChildEntity(args) {
        ...
        var route =
          "/" + args.section + "/" + args.treeAlias + "/edit/" + args.entity.id;
        //change to new path
  ...
  angular
    .module("umbraco.services")
    .factory("umbracoMenuActions", umbracoMenuActions);
  ("use strict");
  /**
   * @ngdoc service
  ...
  function navigationService(
    ...
    var service = {
       ...
       */
      isRouteChangingNavigation: function isRouteChangingNavigation(
        currUrlParams,
        nextUrlParams
      ) {
        ...
        var currRoutingKeys = _.difference(
          _.keys(currUrlParams),
          nonRoutingQueryStrings
        );
        var nextRoutingKeys = _.difference(
          _.keys(nextUrlParams),
          nonRoutingQueryStrings
        );
       ...
       */
      retainQueryStrings: function retainQueryStrings(
        currRouteParams,
        nextRouteParams
      ) {
      ...
      changeSection: function changeSection(sectionAlias, force) {
        ...
        if (
          force &&
          appState.getSectionState("currentSection") === sectionAlias
        ) {
    ...
    */
      ...
      hideTree: function hideTree() {
        if (
          appState.getGlobalState("isTablet") === true &&
          !appState.getGlobalState("stickyNavigation")
        ) {
      ...
      executeMenuAction: function executeMenuAction(action, node, section) {
        ...
        if (
          action.metaData &&
          action.metaData["actionRoute"] &&
          Utilities.isString(action.metaData["actionRoute"])
        ) {
          ...
          action.metaData &&
          action.metaData["jsAction"] &&
          Utilities.isString(action.metaData["jsAction"])
        ) {
          ...
          if (menuAction.length !== 2) {
            ...
            if (!menuActionService) {
              throw (
                "The angular service " + menuAction[0] + " could not be found"
              );
            }
      ...
      showDialog: function showDialog(args) {
        ...
        if (args.action.metaData["dialogTitle"]) {
          appState.setMenuState(
            "dialogTitle",
            args.action.metaData["dialogTitle"]
          );
        } else {
  ...
  angular
    ...
    .factory(
      "notificationsService",
      function ($rootScope, $timeout, angularHelper) {
        var nArray = [];
 ...
 */
  ...
  (function () {
        ...
        push: function push(retryItem) {
          ...
          angular.forEach(service.onItemAddedCallbacks, function (cb) {
            try {
              ...
              $log.error(
                "requestRetryQueue.push(retryItem): callback threw an error" + e
              );
            }
          });
        },
    ...
    angular
      .module("umbraco.services")
      .factory("requestRetryQueue", requestRetryQueue);
  })();
  ("use strict");
  /**
   * @ngdoc service
  ...
  angular
    ...
    .factory(
      "searchService",
      function (entityResource, $injector, searchResultFormatter) {
        return {
          /**
           * @ngdoc method
          ...
          searchMembers: function searchMembers(args) {
            ...
            return entityResource
              .search(args.term, "Member", args.searchFrom)
              .then(function (data) {
            ...
            return entityResource
              ...
              .then(function (data) {
                Object.values(data).forEach(function (resultByType) {
                  ...
                  var formatterMethod =
                    searchResultFormatter.configureDefaultResult;
                  //check if a custom formatter is specified...
                  if (resultByType.jsSvc) {
                    var searchFormatterService = $injector.get(
                      resultByType.jsSvc
                    );
                    if (searchFormatterService) {
                      ...
                      formatterMethod =
                        searchFormatterService[resultByType.jsMethod];
                      if (!formatterMethod) {
                        throw (
                          "The method " +
  ...
  function searchResultFormatter(umbRequestHelper) {
    ...
    function configureContentResult(content, treeAlias, appAlias) {
      content.menuUrl = umbRequestHelper.getApiUrl(
        "contentTreeBaseUrl",
        "GetMenu",
        [{ id: content.id }, { application: appAlias }]
      );
      content.editorPath = appAlias + "/" + treeAlias + "/edit/" + content.id;
      angular.extend(content.metaData, { treeAlias: treeAlias });
    ...
    function configureMemberResult(member, treeAlias, appAlias) {
      member.menuUrl = umbRequestHelper.getApiUrl(
        "memberTreeBaseUrl",
        "GetMenu",
        [{ id: member.id }, { application: appAlias }]
      );
      member.editorPath =
        appAlias +
        "/" +
        treeAlias +
        "/edit/" +
        (member.key ? member.key : member.id);
    ...
    function configureMediaResult(media, treeAlias, appAlias) {
      media.menuUrl = umbRequestHelper.getApiUrl(
        "mediaTreeBaseUrl",
        "GetMenu",
        [{ id: media.id }, { application: appAlias }]
      );
  ...
  angular
    .module("umbraco.services")
    .factory("searchResultFormatter", searchResultFormatter);
  ("use strict");
  /**
   * @ngdoc service
  ...
  function serverValidationManager($timeout) {
    ...
    function getFieldErrors(fieldName) {
      ...
      return _.filter(errorMsgItems, function (item) {
        return (
          item.propertyAlias === null &&
          item.culture === "invariant" &&
    ...
    function getPropertyErrors(
      ...
      return _.filter(errorMsgItems, function (errMsgItem) {
        ...
        var matchProp = matchErrMsgItemProperty(
          errMsgItem,
          propertyAlias,
          options
        );
        return (
          ...
          errMsgItem.segment === segment && // ignore field matching if match options are used
          (options.matchType ||
            errMsgItem.fieldName === fieldName ||
            fieldName === undefined ||
            fieldName === "")
        );
      });
     ...
     */
    function matchErrMsgItemProperty(
      errMsgItem,
      ...
      if (
        ...
        errMsgItem.propertyAlias.startsWith(propertyValidationKey + "/")
      ) {
        return true;
      }
      if (
        options.matchType === "suffix" &&
        errMsgItem.propertyAlias.endsWith("/" + propertyValidationKey)
      ) {
        return true;
      }
      if (
        options.matchType === "contains" &&
        errMsgItem.propertyAlias.includes("/" + propertyValidationKey + "/")
      ) {
        return true;
      }
      return false;
    ...
    function parseComplexEditorError(errorMsg, parentPropertyAlias) {
      ...
      function iterateErrorBlocks(blocks, parentPath) {
        for (var i = 0; i < blocks.length; i++) {
          ...
          for (
            var _i = 0, _Object$entries = Object.entries(nested);
            _i < _Object$entries.length;
            _i++
          ) {
            var _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2),
            ...
            if (Array.isArray(value)) {
              iterateErrorBlocks(
                value,
                "".concat(ms.validationPath, "/").concat(key)
              ); // recurse
            }
          }
        }
    ...
    function getPropertyCallbacks(propertyAlias, culture, fieldName, segment) {
      ...
      var found = _.filter(callbacks, function (cb) {
        ...
        return (
          matchProp &&
          cb.culture === culture &&
          cb.segment === segment && // ignore field matching if match options are used
          (cb.options.matchType ||
            cb.fieldName === fieldName ||
            cb.fieldName === undefined ||
            cb.fieldName === "")
        );
      });
      return found;
    ...
    function matchCallbackItemProperty(cb, propertyValidationKey) {
      ...
      if (
        ...
        propertyValidationKey.startsWith(cb.propertyAlias + "/")
      ) {
        return true;
      }
      if (
        cb.options.matchType === "suffix" &&
        propertyValidationKey.endsWith("/" + cb.propertyAlias)
      ) {
        return true;
      }
      if (
        cb.options.matchType === "contains" &&
        propertyValidationKey.includes("/" + cb.propertyAlias + "/")
      ) {
        return true;
      }
      return false;
    ...
    function addErrorsForModelState(modelState, parentValidationPath) {
      ...
      for (
        var _i2 = 0, _Object$entries2 = Object.entries(modelState);
        _i2 < _Object$entries2.length;
        _i2++
      ) {
        var _Object$entries2$_i = _slicedToArray(_Object$entries2[_i2], 2),
    ...
    function createPropertyValidationKey(propertyAlias, parentValidationPath) {
      return parentValidationPath
        ? parentValidationPath + "/" + propertyAlias
        : propertyAlias;
    }
    /**
     * @ngdoc function
    ...
    var instance = {
       ...
       */
      notifyAndClearAllSubscriptions:
        function notifyAndClearAllSubscriptions() {
          $timeout(function () {
            notifyCallbacks();
        ...
        if (propertyAlias === null) {
          ...
          callbacks = _.reject(callbacks, function (item) {
            return (
              ...
              item.segment === segment &&
              (item.fieldName === fieldName ||
                ((item.fieldName === undefined || item.fieldName === "") &&
       ...
       */
      removePropertyError: function removePropertyError(
        propertyAlias,
        culture,
        fieldName,
        segment,
      ...
      getPropertyErrorsByValidationPath:
        function getPropertyErrorsByValidationPath(
  ...
  angular
    .module("umbraco.services")
    .factory("serverValidationManager", serverValidationManager);
  ("use strict");
  ...
  (function () {
    ...
    angular
      .module("umbraco.services")
      .factory("tabbableService", tabbableService);
  })();
  ("use strict");
  (function () {
    "use strict";
    ...
    angular
      .module("umbraco.services")
      .factory("templateHelper", templateHelperService);
  })();
  ("use strict");
  /**
   * @ngdoc service
  ...
  function tinyMceService(
    ...
    var extendedValidElements =
      "@[id|class|style],-div[id|dir|class|align|style],ins[datetime|cite],-ul[class|style],-li[class|style],-h1[id|dir|class|align|style],-h2[id|dir|class|align|style],-h3[id|dir|class|align|style],-h4[id|dir|class|align|style],-h5[id|dir|class|align|style],-h6[id|style|dir|class|align],span[id|class|style]";
    var fallbackStyles = [
      {
        title: "Page header",
        block: "h2",
    ...
    function getStyles(configuredStylesheets) {
      ...
      if (configuredStylesheets) {
        configuredStylesheets.forEach(function (val, key) {
          if (
            val.indexOf(
              Umbraco.Sys.ServerVariables.umbracoSettings.cssPath + "/"
            ) === 0
          ) {
            ...
            stylesheetResource.getRulesByName(val).then(function (rules) {
              rules.forEach(function (rule) {
                ...
                if (rule.selector[0] == ".") {
                  ...
                  r.block = split[0];
                  r.classes = rule.selector
                    .substring(rule.selector.indexOf(".") + 1)
                    .replace(/\./g, " ");
                } else if (
                  rule.selector[0] != "#" &&
                  rule.selector.indexOf("#") > -1
                  ...
                  var split = rule.selector.split("#");
                  r.block = split[0];
                  r.classes = rule.selector.substring(
                    rule.selector.indexOf("#") + 1
                  );
                } else {
    ...
    function getToolbars(configuredToolbar, tinyMceConfig) {
      ...
      var allowedSelectionToolbar = _.map(
        _.filter(tinyMceConfig.commands, function (f) {
      ...
      var allowedInsertToolbar = _.map(
        _.filter(tinyMceConfig.commands, function (f) {
    ...
    function uploadImageHandler(blobInfo, success, failure, progress) {
      ...
      xhr.open(
        "POST",
        Umbraco.Sys.ServerVariables.umbracoUrls.tinyMceApiBaseUrl +
          "UploadImage"
      );
      ...
      xhr.onload = function () {
        ...
        localStorageService.set(
          "tinymce__".concat(blobInfo.blobUri()),
          json.tmpLocation
        );
    ...
    function sizeImageInEditor(editor, imageDomElement, imgUrl) {
      ...
      if (editor.settings.maxImageSize && editor.settings.maxImageSize !== 0) {
        var newSize = imageHelper.scaleToMaxSize(
          editor.settings.maxImageSize,
          size.w,
          size.h
        );
        ...
        if (imgUrl) {
          var src =
            imgUrl + "?width=" + newSize.width + "&height=" + newSize.height;
    ...
    return {
      ...
      getTinyMceEditorConfig: function getTinyMceEditorConfig(args) {
        ...
        var promises = [this.configuration(), getStyles(args.stylesheets)];
              ...
              if (val) {
                ...
                if (val.detectIsJson()) {
                  try {
                    ...
                    if (
                      Utilities.isArray(config[i]) &&
                      Utilities.isArray(tinyMceConfig.customConfig[i])
                    ) {
                      //concat it and below this concat'd array will overwrite the baseline in angular.extend
                      tinyMceConfig.customConfig[i] = config[i].concat(
                        tinyMceConfig.customConfig[i]
                      );
                    }
                  } catch (e) {}
                }
      ...
      configuration: function configuration() {
        return umbRequestHelper.resourcePromise(
          $http.get(
            umbRequestHelper.getApiUrl("rteApiBaseUrl", "GetConfiguration"),
            { cache: true }
          ),
          "Failed to retrieve tinymce configuration"
        );
       ...
       */
      createInsertEmbeddedMedia: function createInsertEmbeddedMedia(
        editor,
        callback
      ) {
        editor.addButton("umbembeddialog", {
          ...
          onclick: function onclick() {
            ...
            if (
              ...
              selectedElm.classList.contains("embeditem")
            ) {
              // See if we can go and get the attributes
              var embedUrl = editor.dom.getAttrib(
                selectedElm,
                "data-embed-url"
              );
              var embedWidth = editor.dom.getAttrib(
                selectedElm,
                "data-embed-width"
              );
              var embedHeight = editor.dom.getAttrib(
                selectedElm,
                "data-embed-height"
              );
              var embedConstrain = editor.dom.getAttrib(
                selectedElm,
                "data-embed-constrain"
              );
              modify = {
                url: embedUrl,
      ...
      insertEmbeddedMediaInEditor: function insertEmbeddedMediaInEditor(
        editor,
        embed,
        activeElement
      ) {
        ...
        var wrapper = tinymce.activeEditor.dom.create(
          "div",
          {
      ...
      insertMediaInEditor: function insertMediaInEditor(
        editor,
        img,
        imgDomElement
      ) {
      ...
      createInsertMacro: function createInsertMacro(editor, callback) {
        ...
        editor.on("SetContent", function (o) {
          ...
          $(editor.dom.select(".umb-macro-holder.mceNonEditable")).each(
            function () {
      ...
      insertMacroInEditor: function insertMacroInEditor(
        editor,
        macroObject,
        activeMacroElement
      ) {
        ...
        var macroDiv = editor.dom.create(
          "div",
          {
            class:
              "umb-macro-holder " +
              macroObject.macroAlias +
              " " +
              uniqueId +
              " mceNonEditable",
            contenteditable: "false",
        ...
        var $macroDiv = $(
          editor.dom.select("div.umb-macro-holder." + uniqueId)
        );
      ...
      loadMacroContent: function loadMacroContent(
        $macroDiv,
        macroData,
        editor
      ) {
        ...
        angularHelper.safeApply($rootScope, function () {
          macroResource
            .getMacroResultAsHtmlForEditor(
              ...
              contentId,
              macroData.macroParamsDictionary
            )
        ...
        function showDialog(linkList) {
          ...
          if (anchorElm) {
            ...
            if (currentTarget.url.indexOf("localLink:") > 0) {
              ...
              var linkId = currentTarget.url.substring(
                currentTarget.url.indexOf(":") + 1,
                currentTarget.url.lastIndexOf("}")
              );
      ...
      insertLinkInEditor: function insertLinkInEditor(
        editor,
        target,
        anchorElm
      ) {
        ...
        if (
          ...
          target.anchor =
            (target.anchor.indexOf("=") === -1 ? "#" : "?") + target.anchor;
        }
      ...
      initializeEditor: function initializeEditor(args) {
        ...
        args.editor.on("SetContent", function (e) {
          ...
          if (content.search(/src=["']blob:.*?["']/gi) !== -1) {
            ...
            var blobImageWithNoTmpImgAttribute = args.editor.dom.select(
              "img[src^='blob:']:not([data-tmpimg])"
            );
            ...
            blobImageWithNoTmpImgAttribute.forEach(function (imageElement) {
              ...
              var tmpLocation = localStorageService.get(
                "tinymce__".concat(blobSrcUri)
              );
              if (tmpLocation) {
                sizeImageInEditor(args.editor, imageElement);
                args.editor.dom.setAttrib(
                  imageElement,
                  "data-tmpimg",
                  tmpLocation
                );
        ...
        var self = this;
        //create link picker
        self.createLinkPicker(
          args.editor,
          function (currentTarget, anchorElement) {
            entityResource
              .getAnchors(args.model.value)
              .then(function (anchorValues) {
                var linkPicker = {
        ...
        self.createMediaPicker(
          args.editor,
          function (currentTarget, userData, imgDomElement) {
        ...
        self.createInsertEmbeddedMedia(
          args.editor,
          function (activeElement, modify) {
            var embed = {
              modify: modify,
              submit: function submit(model) {
                self.insertEmbeddedMediaInEditor(
                  args.editor,
                  model.embed,
                  activeElement
                );
        ...
        self.createInsertMacro(args.editor, function (dialogData) {
          var macroPicker = {
            ...
            submit: function submit(model) {
              var macroObject = macroService.collectValueData(
                model.selectedMacro,
                model.macroParams,
                dialogData.renderingEngine
              );
              self.insertMacroInEditor(
                args.editor,
                macroObject,
                dialogData.activeMacroElement
              );
              editorService.close();
            },
            close: function close() {
 ...
 */
      ...
      function validateTourRegistration(tour) {
        ...
        Utilities.forEach(tours, function (existingTour) {
          if (existingTour.alias === tour.alias) {
            throw (
              "A tour with the alias " + tour.alias + " is already registered"
            );
          }
        });
      }
  ...
  function treeService(
    ...
    return {
      ...
      _trackExpandedPaths: function _trackExpandedPaths(node, expandedPaths) {
        if (
          !node.children ||
          !Utilities.isArray(node.children) ||
      ...
      _formatNodeDataForUseInUI: function _formatNodeDataForUseInUI(
        parentNode,
        treeNodes,
        section,
        level
        ...
        for (var i = 0; i < treeNodes.length; i++) {
          ...
          if (treeNode.iconIsClass === undefined || treeNode.iconIsClass) {
            ...
            treeNode.style =
              "background-image: url('" + treeNode.iconFilePath + "');";
            //we need an 'icon-' class in there for certain styles to work so if it is image based we'll add this
      ...
      loadNodeChildren: function loadNodeChildren(args) {
        ...
        return this.getChildren(args).then(
          function (data) {
          ...
          function (reason) {
            ...
            eventsService.emit("treeService.treeNodeLoadError", {
              error: reason,
            });
      ...
      getDescendantNode: function getDescendantNode(treeNode, id, treeAlias) {
        ...
        if (!foundRoot) {
          throw (
            "Could not find a tree in the current section with alias " +
            treeAlias
          );
        }
        treeNode = foundRoot;
        ...
        for (var i = 0; i < treeNode.children.length; i++) {
          ...
          if (
            child.children &&
            Utilities.isArray(child.children) &&
      ...
      getTree: function getTree(args) {
        ...
        return treeResource.loadApplication(args).then(function (data) {
          ...
          //format the root
          self._formatNodeDataForUseInUI(
            result.root,
      ...
      getMenu: function getMenu(args) {
        ...
        return treeResource.loadMenu(args.treeNode).then(function (data) {
          ...
          for (var i = 0; i < data.length; i++) {
            data[i].cssclass = iconHelper.convertFromLegacyIcon(
              data[i].cssclass
            );
          }
          return data;
            ...
            if (found) {
              ...
              if (
                Utilities.isFunction(
                  node.parent().children[index].updateNodeData
                )
              ) {
      ...
      getPath: function getPath(node) {
        ...
        while (current != null) {
          ...
          if (
            (current.metaData && current.metaData["treeAlias"]) ||
            !current.parent
          ) {
            current = null;
          } else {
      ...
      syncTree: function syncTree(args) {
        ...
        if (
          String(args.path[currPathIndex]).toLowerCase() ===
          String(args.node.id).toLowerCase()
        ) {
          if (args.path.length === 1) {
            //return the root
            return $q.when(root);
    ...
    function udiService() {
      return {
        ...
        getKey: function getKey(udi) {
          ...
          var withoutHost = withoutScheme
            .substr(withoutScheme.indexOf("/") + 1)
            .trim();
          if (withoutHost.length !== 32) {
            throw "udi is not 32 chars";
          }
          return ""
            .concat(withoutHost.substr(0, 8), "-")
            .concat(withoutHost.substr(8, 4), "-")
            .concat(withoutHost.substr(12, 4), "-")
            .concat(withoutHost.substr(16, 4), "-")
            .concat(withoutHost.substr(20));
        },
      };
    }
      ...
      return {
        ...
        parse: function parse(input) {
          if (
            !input ||
            typeof input !== "string" ||
            !input.startsWith("umb://")
          )
            return null;
          var lastIndexOfSlash = input
            .substring("umb://".length)
            .lastIndexOf("/");
          var entityType =
            lastIndexOfSlash === -1
              ? input.substring("umb://".length)
              : input.substr("umb://".length, lastIndexOfSlash);
          var value =
            lastIndexOfSlash === -1
              ? null
              : input.substring("umb://".length + lastIndexOfSlash + 1);
          return {
            entityType: entityType,
            value: value,
            toString: function toString() {
              return (
                "umb://" + entityType + (value === null ? "" : "/" + value)
              );
            },
          };
        },
  ...
  (function () {
    ...
    function umbDataFormatter() {
      ...
      return {
        ...
        formatContentTypePostData: function formatContentTypePostData(
          displayModel,
          action
        ) {
          ...
          // TODO: Map these
          saveModel.allowedTemplates = _.map(
            displayModel.allowedTemplates,
            function (t) {
              return t.alias;
            }
          );
          saveModel.defaultTemplate = displayModel.defaultTemplate
            ? displayModel.defaultTemplate.alias
            : null;
        ...
        formatDataTypePostData: function formatDataTypePostData(
          displayModel,
          preValues,
          action
        ) {
        ...
        formatDictionaryPostData: function formatDictionaryPostData(
          dictionary,
          nameIsDirty
        ) {
          var saveModel = {
        ...
        formatUserGroupPostData: function formatUserGroupPostData(
          displayModel,
          action
        ) {
          //create the save model from the display model
          ...
          _.each(currAssignedPermissions, function (nodePermissions, index) {
            ...
            _.each(
              nodePermissions.allowedPermissions,
              function (permission, index) {
                if (permission.checked) {
                  saveAssignedPermissions[nodePermissions.id].push(
        ...
        formatMemberPostData: function formatMemberPostData(
          displayModel,
          action
        ) {
          ...
          _.each(displayModel.tabs, function (tab) {
            _.each(tab.properties, function (prop) {
              var foundAlias = _.find(
                memberProviderPropAliases,
                function (item) {
        ...
        formatMediaPostData: function formatMediaPostData(
          displayModel,
          action
        ) {
        ...
        /** formats the display model used to display the content to the model used to save the content  */
        formatContentPostData: function formatContentPostData(
          displayModel,
          action
        ) {
          //NOTE: the display model inherits from the save model so we can in theory just post up the display model but
        ...
        formatContentGetData: function formatContentGetData(displayModel) {
          ...
          if (displayModel.variants && displayModel.variants.length > 1) {
            ...
            var defaultVariants = _.filter(
              displayModel.variants,
              function (variant) {
              ...
              _.each(defaultVariants, function (defaultVariant) {
                ...
                _.each(otherVariants, function (variant) {
                  _.each(invariantProps, function (invProp) {
                    ...
                    var inheritsCulture =
                      prop.culture === invProp.property.culture &&
                      prop.segment == null &&
                      invProp.property.segment == null;
                    var inheritsSegment =
                      prop.segment === invProp.property.segment &&
                      !prop.culture;
         ...
         */
        formatRelationTypePostData: function formatRelationTypePostData(
          relationType
        ) {
          var saveModel = {
            id: relationType.id,
    ...
    angular
      .module("umbraco.services")
      .factory("umbDataFormatter", umbDataFormatter);
  })();
  ("use strict");
  function _typeof(obj) {
    if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") {
  ...
  function umbRequestHelper(
    ...
    return {
       ...
       */
      convertVirtualToAbsolutePath: function convertVirtualToAbsolutePath(
        virtualPath
      ) {
      ...
      getApiUrl: function getApiUrl(apiName, actionName, queryStrings) {
        if (
          ...
          !Umbraco.Sys ||
          !Umbraco.Sys.ServerVariables ||
          !Umbraco.Sys.ServerVariables["umbracoUrls"]
        ) {
        ...
        return (
          Umbraco.Sys.ServerVariables["umbracoUrls"][apiName] +
          actionName +
          (!queryStrings
            ? ""
            : "?" +
              (Utilities.isString(queryStrings)
                ? queryStrings
                : this.dictionaryToQueryString(queryStrings)))
        );
      },
      /**
       * @ngdoc function
          ...
          function (response) {
            ...
            if (response.status >= 500 && response.status < 600) {
              ...
              if (Umbraco.Sys.ServerVariables["isDebuggingEnabled"] === true) {
                ...
                notificationsService.error(
                  "Server error",
                  "Contact administrator, see log for full details.<br/><i>" +
                    result.errorMsg +
                    "</i>"
                );
              }
      ...
      postSaveContent: function postSaveContent(args) {
        ...
        if (
          args.showNotifications === null ||
          args.showNotifications === undefined
        ) {
          args.showNotifications = true;
        }
        ...
        var activeTabIndex =
          activeTab === undefined ? 0 : _.indexOf(args.content.tabs, activeTab);
        //save the data
        return this.postMultiPartRequest(
          ...
          function (data, formData) {
            ...
            for (var f in args.files) {
              ...
              var fileKey =
                "file_" +
                (file.alias || "").replace(/_/g, "\\_") +
                "_" +
                (file.culture ? file.culture.replace(/_/g, "\\_") : "") +
                "_" +
                (file.segment ? file.segment.replace(/_/g, "\\_") : "");
              if (
                Utilities.isArray(file.metaData) &&
                file.metaData.length > 0
              ) {
                fileKey +=
                  "_" +
                  _.map(file.metaData, function (x) {
          ...
          function (response) {
            ...
            if (response.status >= 500 && response.status < 600) {
              ...
              if (
                ...
                typeof response.data.indexOf === "function" &&
                response.data.indexOf("Maximum request length exceeded") >= 0
              ) {
                notificationsService.error(
                  "Server error",
                  "The uploaded file was too large, check with your site administrator to adjust the maximum size allowed"
                );
              } else if (
                Umbraco.Sys.ServerVariables["isDebuggingEnabled"] === true
              ) {
                //show a ysod dialog
                var error = {
                  errorMsg: "An error occurred",
      ...
      postMultiPartRequest: function postMultiPartRequest(
        url,
        jsonData,
        transformCallback
      ) {
        ...
        return $http({
          ...
          transformRequest: function transformRequest(data) {
            ...
            if (Utilities.isArray(data)) {
              ...
              formData.append(
                data.key,
                !Utilities.isString(data.value)
                  ? Utilities.toJson(data.value)
                  : data.value
              );
            }
            //call the callback
      ...
      downloadFile: function downloadFile(httpPath) {
         ...
         */
        // Use an arraybuffer
        return $http.get(httpPath, { responseType: "arraybuffer" }).then(
          function (response) {
  ...
  angular
    .module("umbraco.services")
    .factory("umbRequestHelper", umbRequestHelper);
  ("use strict");
  /**
   * @ngdoc service
      ...
      return {
        ...
        parseHashIntoUrl: function parseHashIntoUrl(url) {
          ...
          if (!urlObj.hash.startsWith("#/")) {
            throw (
              new "The hash in url does not contain a path to parse: "() + url
            );
          }
  ...
  */
        function countdownUserTimeout() {
          $timeout(
              ...
              if (currentUser) {
                ...
                if (currentUser.remainingAuthSeconds > 30) {
                  ...
                  if (lastServerTimeoutSet != null) {
                    var now = new Date();
                    var seconds =
                      (now.getTime() - lastServerTimeoutSet.getTime()) / 1000;
                    if (seconds > 30) {
            ...
            false
          ); //false = do NOT execute a digest for every iteration
        }
        ...
        return {
            ...
            return authResource
              ...
              .then(
                function (data) {
                  ...
                  if (
                    data.startContentIds.length === 0 &&
                    data.startMediaIds.length === 0
                  ) {
          ...
          setAuthenticationSuccessful: function setAuthenticationSuccessful(
            data
          ) {
          ...
          refreshCurrentUser: function refreshCurrentUser() {
            var deferred = $q.defer();
            authResource.getCurrentUser().then(
              function (data) {
                var result = {
                  user: data,
                  authenticated: true,
          ...
          getCurrentUser: function getCurrentUser(args) {
            if (!currentUser) {
              return authResource.getCurrentUser().then(
                function (data) {
                  var result = {
                    user: data,
                    authenticated: true,
  ...
  (function () {
    ...
    function usersHelperService(localizationService) {
      ...
      localizationService
        .localizeMany(
          userStates.map(function (userState) {
    ...
    angular
      .module("umbraco.services")
      .factory("usersHelper", usersHelperService);
  })();
  ("use strict");
  ...
  function dateHelper() {
    return {
      convertToServerStringTime: function convertToServerStringTime(
        momentLocal,
        serverOffsetMinutes,
        format
      ) {
      ...
      convertToLocalMomentTime: function convertToLocalMomentTime(
        strVal,
        serverOffsetMinutes
      ) {
        //get the formatted offset time in HH:mm (server time offset is in minutes)
        ...
        if (strVal.indexOf("T") > -1 && strVal.endsWith("Z")) {
          isoFormat =
            moment.utc(strVal).format("YYYY-MM-DDTHH:mm:ss") + formattedOffset;
        } else {
          isoFormat =
  ...
  angular
    .module("umbraco.services")
    .factory("umbSessionStorage", umbSessionStorage);
  /**
   * @ngdoc function
  ...
  function updateChecker($http, umbRequestHelper) {
    return {
      ...
      check: function check() {
        return umbRequestHelper.resourcePromise(
          $http.get(
            umbRequestHelper.getApiUrl("updateCheckApiBaseUrl", "GetCheck")
          ),
  ...
  angular
    .module("umbraco.services")
    .factory("umbPropEditorHelper", umbPropEditorHelper);
  ("use strict");
  (function () {
    "use strict";
    ...
    angular
      .module("umbraco.services")
      .factory("validationMessageService", validationMessageService);
  })();
  ("use strict");
  /**
   * @ngdoc service
  ...
  angular
    .module("umbraco.services")
    .factory("windowResizeListener", windowResizeListener);
  ("use strict");
  /**
   * @ngdoc service

(function () {
  ...
  angular
    .module("umbraco.filters")
    .filter("ncNodeName", function (editorState, entityResource) {
      function formatLabel(firstNodeName, totalNodes) {
        return totalNodes <= 1
          ? firstNodeName // If there is more than one item selected, append the additional number of items selected to hint that
  ...
  angular
    .module("umbraco.filters")
    .filter("preserveNewLineInHtml", function () {
  ...
  (function () {
    ...
    angular
      .module("umbraco.filters")
      .filter("umbWordLimit", umbWordLimitFilter);
  })();
})();
